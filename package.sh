...
echo -e "${GREEN}Creating zip archive (Windows)...${NC}"
ZIP_NAME="${PACKAGE_NAME}.zip"
if [ -f "$ZIP_NAME" ]; then rm "$ZIP_NAME"; fi
zip -q -r "$ZIP_NAME" "$PACKAGE_NAME" -x "*.pyc" -x "*/__pycache__/*"
...
echo -e "  Tarball:   ${GREEN}$ARCHIVE_NAME${NC} ($ARCHIVE_SIZE)"
echo -e "  Zip:       ${GREEN}$ZIP_NAME${NC} ($ZIP_SIZE)"
echo ""
